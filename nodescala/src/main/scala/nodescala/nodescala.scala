...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    def responseLoop(ct: CancellationToken): Future[Unit] = {
      ...
      //        }
      //      }
      def resp(t: Try[(NodeScala.Request, Exchange)]): Unit = {
        t match {
          case Success(s) => respond(s._2, ct, handler(s._1))
    ...
    val responseLoopSubscription = Future.run()(responseLoop)
    Subscription(listenerSubscription, responseLoopSubscription)
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      def f(p: Promise[(Request, Exchange)])(e: Exchange): Unit = {
